...
assert(formatSentRequests(payerIncludedRequests).includes('Send this link: https://remy.test/r'), 'sent reply should keep the tracked pay link')
assert(!payerIncludedRequests[0].url.includes('?'), 'new payment links should stay short and id-only')
assert(new URL(payerIncludedRequests[0].url).pathname.split('/').at(-1)?.length === 12, 'new payment links should use compact request ids')
...
try {
  ...
  assert(cardSvg.includes('Request card ready'), 'image-card SVG should render the polished widget copy')
  assert(!cardSvg.includes('tap to pay'), 'payment card image should not imply the image itself is tappable')
